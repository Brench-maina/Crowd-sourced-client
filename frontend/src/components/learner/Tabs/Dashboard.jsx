  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (progressResponse.ok) {
        ...
        setUserProgress(progressData.progress || progressData); 
      }

      if (pathsResponse.ok) {
        const pathsData = await pathsResponse.json();
        setLearningPaths(pathsData.paths || pathsData); 
      }

      if (badgesResponse.ok) {
        const badgesData = await badgesResponse.json();
        setUserBadges(badgesData.badges || badgesData); 
      }

      if (challengesResponse.ok) {
        const challengesData = await challengesResponse.json();
        setActiveChallenges(challengesData.challenges || challengesData); 
      }

      if (leaderboardResponse.ok) {
        const leaderboardData = await leaderboardResponse.json();
        setLeaderboardData(leaderboardData.leaderboard || leaderboardData); 
      }
